...
K_medoids <- function(data, k, initial_method, dist_method) {
  ...
  if (initial_method == "random") {
    print("Please select other initialisation method, this is not sufficient")
    break
  }
...
# Example 2: K-medoids iteration process
example2 <- function(){
  simple.data = Overlapped_AD(150,2,3,0.05,0)
  simple.data.KMD <- K_medoids(simple.data$dataSet, 3, "k-means++", "Manhattan")
  dd = simple.data.KMD$cluster
